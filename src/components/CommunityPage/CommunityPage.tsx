...
export default function CommunityPage() {
    ...
    const fetchData = async () => {
      try {
        const [communityResponse, postsResponse] = await Promise.all([
          axios.get(`http://localhost:8080/communities/${id}`),
          axios.get(`http://localhost:8080/communities/${id}/posts`)
        ]);
  ...
  const handleDeleteCommunity = async () => {
    ...
    try {
      const response = await axios.delete(`http://localhost:8080/communities/${id}`,{headers:{"Authorization": `Bearer ${localStorage.getItem("token")}`}});
              ...
              <div className="flex items-center space-x-4">
                {(isCreator || isAdmin) && (
                  <button
                    ...
                    disabled={isDeleting}
                    className={`px-4 py-2 text-red-600 border border-red-600 rounded-lg hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition duration-200 ${isDeleting ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
